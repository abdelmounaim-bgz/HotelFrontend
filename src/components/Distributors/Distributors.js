...
function Distributors() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://192.168.12.54:3000/usersList/${localStorage.getItem('id')}`, config).then(
      res => {
        setTimeout(() => {
  ...
  const handleEnable = () => {
    ...
    const config = { headers: { Authorization: 'Bearer ' + localStorage.getItem('token')} }
    axios.put(`http://192.168.12.54:3000/usersAttribute/${newpost.id}`, newpost, { config }).then(
      res => {
        forceUpdate();
  ...
  const handleAdd = async () => {
    ...
    if(error == false ){    
      ...
      const user = { username, contact, email, telephone, password, lockSupplier, tokenRefreshedAt, role, date, hotelNumber, subDistributor, enable }

      const config = {  headers: { Authorization: 'Bearer ' + localStorage.getItem('token') }  }
      await axios.post(`http://192.168.12.54:3000/users/${localStorage.getItem('id')}`, user, { config }).then(
        res => {
          forceUpdate();
          setShow(false);
        },
  ...
  const handleDelete = (id) => {
    ...
    axios.delete(`http://192.168.12.54:3000/users/${id}`, config).then(
      res => {
        forceUpdate();
      },
      err => {
  ...
  const handlePut = () => {
    ...
    if(error == false ){  
        const newpost = { id, username, contact, email, telephone, hotelNumber, enable }
        const config = {headers: { Authorization: 'Bearer ' + localStorage.getItem('token') } }
        axios.put(`http://192.168.12.54:3000/usersAttribute/${newpost.id}`, newpost, { config }).then(
          res => {
            forceUpdate();
                                    ...
                                    <td>
                                      <center>
                                        <Dropdown>
                                          ...
                                          <Dropdown.Menu >
                                            {post.enable ? <Dropdown.Item onClick={() => handleOpenConfirmation(post)} href="" style={{ color: '#CE0000' }}> {t("Désactiver")} </Dropdown.Item>
                                              : <Dropdown.Item onClick={() => handleOpenConfirmation(post)} href="" style={{ color: '#0073CE' }}> {t("Activer")} </Dropdown.Item>}
                                            <Dropdown.Item href="" onClick={() => handleModify(post)}> {t("Modifier")} </Dropdown.Item>
                                            <Dropdown.Item href="" style={{ color: '#CE0000' }} onClick={() => handleDelete(post.id)}> {t("Supprimer")} </Dropdown.Item>
